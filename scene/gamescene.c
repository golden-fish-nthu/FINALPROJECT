...
void _Scene_load_map(Scene *pObj)
{
	 FILE *data;
	 ...
	 for (int i = 0; i < 10; i++)
	 {
	      for (int j = 0; j < 16; j++)
	      {
	            fscanf(data, "%d", &pObj->map_data[i][j]);
	      }
	 }
	 fclose(data);
}
...
Scene *New_GameScene(int label)
{
    ...
    pObj->countdowntimerpointer = &pDerivedObj->countdownTime;
    ...
    pDerivedObj->countdownTime = 120.0; // 设置倒计时时长为120秒
    pDerivedObj->timeUp = false;

    pObj->pDerivedObj = pDerivedObj;
	_Scene_load_map(pObj);
    // register element
	...
	for (int i = 0; i < 10; i++)
    {
        for (int j = 0; j < 16; j++)
        {
            switch (pObj->map_data[i][j])
            {
                case 10:
                    _Register_elements(pObj, New_Beef(Beef_L, i, j));
                    break;
                case 13:
                    printf("money added in gamescene\n");
                    _Register_elements(pObj, New_money(Money_L, i, j, pObj));
                    break;
                case 11:
                    _Register_elements(pObj, New_Veget(Veget_L, i, j));
                    break;
                default:
                    break;
            }
    ...
    _Register_elements(pObj, New_guest(Guest_L));
...
void set_count_down_time(GameScene* game_scene, double time)
{
    game_scene->countdownTime = time;
}
