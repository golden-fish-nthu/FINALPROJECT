...
/*
    [Veget function]
*/
Elements *New_Veget(int label, int i, int j)
{
...
void Veget_interact(Elements *self, Elements *tar)
{
     if (tar->label == Character_L)
     {
          ...
          Veget *veget = (Veget *)(self->pDerivedObj);
          int right_limit = WIDTH - chara->width / 2;
          int left_limit = 0 - chara->width / 2;

          if (((chara->x >=  veget->x && chara->x < veget->x + veget->width) || (chara->x + chara->width >= veget->x && chara->x < veget->x + veget->width))
          && ((chara->y >=  veget->y && chara->y < veget->y + veget->height) || (chara->y + chara->height >= veget->y && chara->y < veget->y + veget->height))
          && key_state[ALLEGRO_KEY_E])
          {
              
			if (chara->state == MOVE)
                    chara->state = TAKEVEGE;
		}
